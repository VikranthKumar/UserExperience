...
public struct FeedbackGeneratorEnvironmentKey: EnvironmentKey {
    public static let defaultValue = FeedbackGenerator.shared
}
...
struct FeedbackOnButtonAction: ViewModifier {
    @Environment(\.feedbackGenerator) private var feedbackGenerator
    ...
    func body(content: Content) -> some View {
        content.onAppear {
            self.feedbackGenerator.prepare(self.style)
        }.onButtonAction {
            self.feedbackGenerator.generate(self.style)
        }
    }
...
extension View {
    ...
    public func onButtonAction(_ feedback: FeedbackGenerator.FeedbackStyle) -> some View {
        modifier(FeedbackOnButtonAction(style: feedback))
    }
}
