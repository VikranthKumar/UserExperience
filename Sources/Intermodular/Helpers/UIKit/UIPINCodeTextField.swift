...
@IBDesignable open class UIPINCodeTextField: UIView {
    public weak var delegate: UIPINCodeTextFieldDelegate?
    ...
    override open var intrinsicContentSize: CGSize {
        ...
        width += CGFloat(characterLimit) * underlineWidth
        width += CGFloat((characterLimit > 0) ?  (characterLimit - 1) : 0) * underlineHSpacing
        
        var height: CGFloat = 0
        
        height += font.ascender + font.descender
        height += underlineVMargin + underlineHeight
    ...
    func canInsertCharacter(_ character: String) -> Bool {
        ...
        let isNewline = character.trimmingCharacters(in: CharacterSet.newlines).isEmpty
...
extension UIPINCodeTextField: UIKeyInput {
    ...
    public func insertText(_ charToInsert: String) {
        if charToInsert.trimmingCharacters(in: CharacterSet.newlines).isEmpty {
